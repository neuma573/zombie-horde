...
export function totalAvailableAmmo(
  ...
  const ammoTypes = new Set(owned.flatMap((weapon) => (
    weapon.definition.ammoType === null ? [] : [weapon.definition.ammoType]
  )));
  ...
  const reserveCapacityByAmmoType = owned.reduce((capacities, weapon) => {
    ...
    if (ammoType === null) return capacities;
    capacities.set(
      ammoType,
      Math.max(
...
): { state: SupplyTriggerState; shouldDrop: boolean; chance: number } {
  if (input.activeSupply) return { state, shouldDrop: false, chance: 0 };
  if (!input.waveCleared) return { state, shouldDrop: false, chance: 0 };
...
export function selectSupplyDropLocation(
  ...
  const eligible = reachable.filter((candidate) => {
    ...
    return playerDistance >= config.normalMinimumPlayerDistance
  ...
  return pool.reduce((best, candidate) => {
    const score = locationScore(candidate, seed);
...
function locationScore(candidate: Position, seed: number): number {
  return mixUint32(
