...
import { t, userSettings, setLanguage } from '../systems/UserSettings';
...
export class AssetDebugScene extends Phaser.Scene {
  ...
  private createEntries(): Entry[] {
    const entries: Entry[] = Object.values(WEAPON_DEFINITIONS).map((weapon) => ({
      ...
      description: () => [t(weapon.description), t(weapon.rarity.toUpperCase()),
        t('Damage {amount}', { amount: weapon.config.damage }),
        t('Range {amount}', { amount: weapon.config.range }),
        ...weaponTooltipStatLines(weaponTooltipStats(weapon, userSettings.locale), userSettings.locale),
      ].join('\n'),
      preview: () => {
    ...
    for (const kind of Object.keys(DEBUG_CONSUMABLES) as Array<keyof typeof DEBUG_CONSUMABLES>) {
      ...
      entries.push({ id: kind, name: item.name, category: 'Items',
        description: () => t(kind === 'medical' ? 'Healing {amount}' : 'Ammo {amount}', { amount: item.amount }),
    ...
    for (const kind of ['normal', 'fast'] as const) {
      for (let variant = 0; variant < 12; variant++) {
        entries.push({ id: `${kind}-${variant}`, name: `${t(kind === 'normal' ? 'Zombie' : 'Fast zombie')} ${variant + 1}`, category: 'Models & motion',
    ...
    for (const name of ['Shot', 'Zombie hit', 'Zombie death', 'Player hit', 'Crate hit', 'Crate destroyed', 'Melee contact']) {
      entries.push({ id: `effect-${name}`, name, category: 'Models & motion', preview: () => {
        ...
        play(); this.details?.append(this.button(t(name), play));
      } });
    }
    URBAN_MAP_CONFIG.obstacles.forEach((config, index) => {
      entries.push({ id: `building-${index}`, name: `${t('Building')} ${index + 1}`, category: 'Models & motion',
        preview: () => {
  ...
  private createPanel(): void {
    ...
    this.panel = panel;
    const heading = document.createElement('h1'); heading.textContent = t('ASSET DEBUG'); heading.style.margin = '0 0 8px';
    const toolbar = document.createElement('div'); toolbar.style.cssText = 'display:flex;gap:8px;flex-wrap:wrap;align-items:center';
    const exit = this.button(t('MAIN MENU'), () => {
      const url = new URL(location.href); url.searchParams.delete('debug'); history.replaceState({}, '', url);
      this.scene.start('MainMenuScene');
    });
    const language = this.select(['en', 'ko'], userSettings.locale, (value) => {
      setLanguage(value === 'ko' ? 'ko' : 'en'); this.entries = this.createEntries(); this.createPanel();
    }, (value) => value === 'en' ? 'English' : '한국어');
    ...
    filter.setAttribute('aria-label', t('Category'));
    const search = document.createElement('input'); search.type = 'search'; search.placeholder = t('Search'); search.setAttribute('aria-label', t('Search')); search.value = this.query;
    search.oninput = () => { this.query = search.value; this.renderResults(); };
    toolbar.append(exit, language, filter, search);
  ...
  private renderResults(): void {
    ...
    const filtered = this.entries.filter((entry) => (this.category === 'All' || this.category === entry.category)
      && `${entry.id} ${entry.name} ${t(entry.name)}`.toLocaleLowerCase().includes(query));
    for (const entry of filtered) this.results?.append(this.button(t(entry.name), () => this.selectEntry(entry)));
    if (!filtered.length && this.results) this.results.textContent = t('No results');
  }

  private selectEntry(entry: Entry): void {
    this.selected = entry;
    this.details?.replaceChildren();
    const title = document.createElement('h2'); title.textContent = t(entry.name); title.style.margin = '8px 0';
    ...
    if (entry.url) {
      if (entry.category === 'Sounds') {
        ...
        const img = document.createElement('img'); img.src = entry.url; img.alt = t(entry.name); img.style.cssText = 'max-width:100%;max-height:22vh;object-fit:contain;background:repeating-conic-gradient(#444 0% 25%,#777 0% 50%) 0/20px 20px'; this.details?.append(img);
      }
    }
    this.refreshPreview();
  ...
  private addMotionControls(): void {
    ...
    direction.setAttribute('aria-label', t('Direction'));
    const speed = this.select(['0.1','0.25','0.5','1'], String(this.speed), (value) => { this.speed = Number(value); }, (value) => t('{speed}×', { speed: value }));
    speed.setAttribute('aria-label', t('Playback speed'));
    controls.append(this.button(t('Play / Pause'), () => { this.playing = !this.playing; }),
      this.button(t('Movement'), () => { this.moving = !this.moving; }), direction, speed);
    if (this.player) {
      ...
      }, (value) => t(WEAPON_DEFINITIONS[value as WeaponId].name));
      weapon.setAttribute('aria-label', t('Weapon'));
      ...
      controls.append(weapon, slider, this.button(t('Muzzle flash'), () => this.player?.triggerMuzzleReflection()),
        this.button(t('Recoil'), () => this.player?.triggerWeaponRecoil(7)),
        this.button(t('Reload animation'), () => { this.reloading = !this.reloading; }));
    }
  ...
  private select(values: string[], current: string, action: (value: string) => void, label: (value: string) => string = t): HTMLSelectElement {
