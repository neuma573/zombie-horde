...
describe('melee input timeline wiring', () => {
  it('queues pointer fire until the current fixed steps have been consumed', async () => {
    ...
    const updateStart = gameScene.indexOf('  update(_time: number, deltaMs: number): void {');
    ...
    const fireResolution = update.indexOf('this.resolveFireRequests();');
    ...
    expect(pointerDown).toContain('this.playerInput = requestFire(this.playerInput);');
    expect(pointerDown).not.toContain('this.resolveFireRequests();');
    expect(fixedStepLoop).toBeGreaterThanOrEqual(0);
    expect(fireResolution).toBeGreaterThan(fixedStepLoop);
  });
});
