...
describe('weapon accuracy proof', () => {
  it('proves the rifle first shot is more accurate across randomized recoil seeds', () => {
    const pistol = sampleAccuracy(PISTOL_WEAPON, 0, true);
    const rifle = sampleAccuracy(BURST_RIFLE_WEAPON, 0, true);

    console.info({
      samples: SAMPLE_COUNT,
      targetHalfAngleDegrees: SMALL_TARGET_HALF_ANGLE_DEGREES,
      pistolFirstShot: pistol,
      rifleFirstShot: rifle,
    });
  ...
  it('proves sustained rifle fire eventually becomes harder to control', () => {
    ...
    console.info({
      samples: SAMPLE_COUNT,
      pistolSixthShot: pistol,
      rifleSixthShot: rifle,
    });
