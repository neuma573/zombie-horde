...
- 테스트를 작성하거나 변경할 때는 `docs/TESTING.md`의 테스트 계층, 배치, 작성 및 검증 규칙을 따른다.
- AGENTS.md의 요구사항이 서로 충돌하면 구현 전에 충돌 지점을 보고하고 임의로 해석하지 않는다.
- 구현은 현재 브랜치와 마일스톤의 목표 범위를 따른다.
- 향후 기능은 확장 가능한 구조를 미리 준비할 수 있다.
- 기능은 현재 브랜치의 목표에 포함된 경우 런타임에 활성화한다.
...
- 무기는 단발, 점사, 연사, 볼트액션, 펌프액션 방식을 지원할 수 있다.
...
- 모바일 환경에서는 세로 모드를 기본으로 하며, 세로와 가로 모드 모두에서 플레이 영역이 반응형으로 동작해야 한다.
