...
- 게임 HUD의 공식 최소 사용 영역은 safe-area를 제외한 CSS 기준 `320×360`이다.
- 일반 모바일 검증은 세로 `390×844`와 가로 `844×390`을 우선한다.
- 최소 범위보다 작은 viewport에서도 좌표 계산은 안전하게 유지하지만, 모든 HUD의
  가독성과 48px 터치 타깃을 동시에 보장하지는 않는다.
- PC와 모바일 입력 모드 전환은 HUD, PAUSE 진입 버튼과 모바일 조작계 레이아웃을
  하나의 UI 전환으로 다시 적용한다.
...
- 수동 조준 결과를 명확히 제공해 향후 모바일 자동 조준을 그 다음 단계로 추가할 수 있게 한다.
- 이번 작업에서는 적 가시성, 자동 조준, 타깃 잠금, 자동 사격을 구현하지 않는다.
...
향후 자동 조준은 `수동 입력 의도 → 가시성/자동 조준 보정 → 최종 조준 방향` 사이에 추가한다. 이번 작업에는 자동 조준 결과 타입 또는 빈 시스템을 만들지 않는다.
...
- 월드의 무기 픽업 아이콘이 스틱이나 조작 버튼과 화면상 겹쳐도 모바일 control 역할을 먼저 판정한다. control 영역의 터치는 픽업 이벤트가 소비하지 않는다.
...
이번 작업은 첫 흐름만 구현한다. 입력 계층은 Zombie 목록, Wave 또는 자동 조준 상태를 참조하지 않는다.
...
- 무기 픽업 아이콘과 이동 스틱이 겹친 상태에서 이동 조작
