...
- 지도는 중앙에서 기본 채움 배율보다 15% 확대해 시작한다. 지도 아래에 짧은 드래그 안내를 표시하고, PC에서는 잡기 커서로 이동 가능함을 알린다. 방향 버튼은 표시하지 않는다. 드래그·핀치·휠 조작은 유지한다.
- 건물 선택 시 현재 배율과 무관하게 고정 1.25배율까지 700ms 동안 부드럽게 전환하고, 해제 시 선택 전 배율로 돌아온다. 직접 지도를 조작한 뒤 해제하면 초기 배율로 돌아간다. 다른 건물을 연속 선택해도 확대를 누적하지 않으며, 직접 드래그·핀치·휠 입력은 연출보다 우선한다.
- 선택한 건물을 지도 뷰포트 중앙으로 함께 이동한다. 지도 경계에 가까운 건물은 바깥 여백이 드러나지 않는 범위에서 중앙에 가깝게 배치한다. 선택 표시는 기존 펜 동그라미와 글자색만 사용하며, 별도 채움색·강조 이름표·주변 흐림 효과는 적용하지 않는다.
- 건물 상세 메모에는 이름, 탐색 소요 시간, 선택 상태만 표시한다. 식량·탄약·연료의 낮음·높음 등 예상치는 표시하지 않는다.
- 탐색 지점은 기존 6곳에 식당, 정비소, 철물점, 창고, 대형마트, 연료 저장소를 추가한 총 12곳이다. 새 지점도 기존 12시간 배분과 자원 확률 규칙을 따른다.
...
- ‘오늘 계획 일괄 확정’을 누르면 탐색 장소별 소요 시간과 총 수색·수리·휴식 시간을 요약하고 ‘이 계획으로 진행할까요?’라고 한 번 확인한다. ‘돌아가기’는 배정한 계획을 그대로 보존하며, ‘진행’을 눌렀을 때만 탐색·수리 결과를 확정한다. 확인창이 열린 동안 배경 조작을 막고, 화면 크기가 바뀌어도 확인 상태를 유지한다. 개별 건물 선택이나 수리 시간 변경에는 확인창을 띄우지 않는다.
- 수리만 배정할 수 있다. 빈 계획은 확정할 수 없다.
- 탐색 버튼과 메인 메뉴 링크는 해당 조작 영역에서 누르기 시작한 탭만 처리한다. 지도 드래그를 버튼 위에서 놓아도 계획 확정이나 화면 이동을 실행하지 않는다.
- 장소 또는 수리 시간이 계획에 배정되어 있는 동안 직접 수색 API는 `PLAN ACTIVE`로 거부한다. 배정된 계획은 `confirmPlan()`으로 일괄 확정하며, 배정을 모두 해제하면 직접 수색을 다시 사용할 수 있다.
- 결과에는 합산 자원, 수색 건물 수, 작업 소요 시간, 수리량과 현재 바리케이드를 표시한다. 수색·수리에 배정하지 않은 남은 시간은 휴식으로 보고하며, 별도 자원이나 회복 효과는 부여하지 않는다.
- 우측 상단에 현재 Day를 표시하고 12칸 시간 막대로 수색·수리·휴식을 구분한다. 계획 단계의 휴식 시간은 건물 선택이나 수리에 재배분할 수 있다. 시간 초과 선택 시 지도 위에 필요/미배정 시간 경고를 표시한다.
...
- 안전 영역을 제외한 화면이 320×360 이상이면 무기고 조작 UI는 원래 크기를 유지한다. 높이가 짧고 좁은 화면에서는 44px 슬롯 안에 슬롯 이름과 장착 상태를 두 줄로 표시하며, 방어 시작 버튼도 높이 44px를 유지한다. 얕은 진열대에서는 초기 배율만 낮춰 권총 전체를 표시한다. 이보다 작은 화면은 기존 축소 배치를 사용한다.
